...
void TridimConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void TridimConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    if (propagate_down[0]) {
      ...
      col2vol_gpu(col_diff, channels_, length_, height_, width_, kernel_size_, kernel_d_, pad_,
          pad_d_, stride_, stride_d_, bottom_diff + bottom[0]->offset(offset_indices));
    }
  }
}

INSTANTIATE_LAYER_GPU_FUNCS(TridimConvolutionLayer);
