...
#include "caffe/layers/tridim_data_layer.hpp"
...
TridimDataLayer<Dtype>::~TridimDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void TridimDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int new_length = this->layer_param_.tridim_data_param().new_length();
  const int new_height = this->layer_param_.tridim_data_param().new_height();
  const int new_width  = this->layer_param_.tridim_data_param().new_width();
  string root_folder = this->layer_param_.tridim_data_param().root_folder();
  ...
  // Read the list file
  const string& source = this->layer_param_.tridim_data_param().source();
  const bool use_temporal_jitter = this->layer_param_.tridim_data_param().use_temporal_jitter();
  const bool use_image = this->layer_param_.tridim_data_param().use_image();
  const int sampling_rate = this->layer_param_.tridim_data_param().sampling_rate();
  const bool use_multiple_label = this->layer_param_.tridim_data_param().use_multiple_label();
  if (use_multiple_label) {
    CHECK(this->layer_param_.tridim_data_param().has_num_of_labels()) <<
    "number of labels must be set together with use multiple labels";

  }
  const int num_of_labels = this->layer_param_.tridim_data_param().num_of_labels();
  ...
  if (this->layer_param_.tridim_data_param().shuffle()) {
  ...
  lines_id_ = 0;
  LOG(INFO) << "A total of " << shuffle_index_.size() << " tridim chunks.";

  // Check if we would need to randomly skip a few data points
  if (this->layer_param_.tridim_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
        this->layer_param_.tridim_data_param().rand_skip();
  ...
  const int batch_size = this->layer_param_.tridim_data_param().batch_size();
...
template <typename Dtype>
void TridimDataLayer<Dtype>::ShuffleClips() {
...
template <typename Dtype>
void TridimDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  TridimDataParameter tridim_data_param = this->layer_param_.tridim_data_param();
  const int batch_size = tridim_data_param.batch_size();
  const int new_length = tridim_data_param.new_length();
  const int new_height = tridim_data_param.new_height();
  const int new_width = tridim_data_param.new_width();
  string root_folder = tridim_data_param.root_folder();
  const bool use_image = tridim_data_param.use_image();
  const bool use_temporal_jitter = tridim_data_param.use_temporal_jitter();
  int sampling_rate = tridim_data_param.sampling_rate();
  const int max_sampling_rate = tridim_data_param.max_sampling_rate();
  const bool use_sampling_rate_jitter = tridim_data_param.use_sampling_rate_jitter();
  const bool show_data = tridim_data_param.show_data();
  ...
  if (!use_image){
   ...
   // LOG(INFO) << "read tridim from " << file_list_[id].c_str();
   CHECK(ReadImageSequenceToVolumeDatum((root_folder + file_list_[id]).c_str(), start_frm_list_[id], label_list_[id],
                              new_length, new_height, new_width, sampling_rate, &datum));
  }
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    // Apply transformations (mirror, crop...) to the tridim
...
INSTANTIATE_CLASS(TridimDataLayer);
REGISTER_LAYER_CLASS(TridimData);
