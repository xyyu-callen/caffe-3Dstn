...
#include "caffe/layers/tridim_convolution_layer.hpp"
...
void TridimConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
	// already done in LayersetUp
}

template <typename Dtype>
void TridimConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  kernel_size_ = this->layer_param_.tridim_convolution_param().kernel_size();
  kernel_d_ = this->layer_param_.tridim_convolution_param().kernel_size();
  //kernel_d_ = this->layer_param_.tridim_convolution_param().kernel_d();
  stride_ = this->layer_param_.tridim_convolution_param().stride();
  stride_d_ = this->layer_param_.tridim_convolution_param().stride();
  //stride_d_ = this->layer_param_.tridim_convolution_param().stride_d();
  pad_ = this->layer_param_.tridim_convolution_param().pad();
  pad_d_ = this->layer_param_.tridim_convolution_param().pad();
  //pad_d_ = this->layer_param_.tridim_convolution_param().pad_d();
  num_ = bottom[0]->shape(0);
  channels_ = bottom[0]->shape(1);
  length_ = bottom[0]->shape(2);
  height_ = bottom[0]->shape(3);
  width_ = bottom[0]->shape(4);
  num_output_ = this->layer_param_.tridim_convolution_param().num_output();
  filter_group_ = this->layer_param_.tridim_convolution_param().filter_group();
  ...
  int length_out = (length_ + 2 * pad_d_ - kernel_d_) / stride_d_ + 1;
  ...
  col_shape[0] = 1;
  col_shape[1] = channels_ * kernel_d_ * kernel_size_ * kernel_size_;
  ...
  col_buffer_.Reshape(col_shape);
      //1, channels_ * kernel_d_ * kernel_size_ * kernel_size_, length_out, height_out, width_out);


  bias_term_ = this->layer_param_.tridim_convolution_param().bias_term();

  // Figure out the dimensions for individual gemms.
  M_ = num_output_ / filter_group_; // doing convolution filter_group_ times per volume
  K_ = channels_ * kernel_d_ * kernel_size_ * kernel_size_;
  ...
  if (this->blobs_.size() > 0) {
    ...
    weight_shape[2] = kernel_d_;
    weight_shape[3] = kernel_size_;
    weight_shape[4] = kernel_size_;
    // Initialize the weights
    this->blobs_[0].reset(new Blob<Dtype>(weight_shape));
        //num_output_, channels_, kernel_d_, kernel_size_, kernel_size_));

    // fill the weights
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.tridim_convolution_param().weight_filler()));
    ...
    if (bias_term_) {
      ...
      shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
          this->layer_param_.tridim_convolution_param().bias_filler()));
      bias_filler->Fill(this->blobs_[1].get());
    }
    // profiling number of params
    std::ofstream out("profile.log", std::ofstream::out | std::ofstream::app);
    out << this->layer_param_.name() << " " << num_output_ * channels_ * kernel_d_ * kernel_size_ * kernel_size_ + num_output_ << "\n";
    out.close();
  }
...
template <typename Dtype>
void TridimConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	    const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    vol2col_cpu(bottom_data + bottom[0]->offset(offset_indices), channels_, length_, height_,
	    		  width_, kernel_size_, kernel_d_, pad_, pad_d_, stride_, stride_d_, col_data);
...
template <typename Dtype>
void TridimConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < num_; ++n) {
	...
	vol2col_cpu(bottom_data + bottom[0]->offset(offset_indices), channels_, length_, height_,
					  width_, kernel_size_, kernel_d_, pad_, pad_d_, stride_,
					  stride_d_, col_data);
	...
	if (propagate_down[0]) {
	  ...
	  col2vol_cpu(col_diff, channels_, length_, height_, width_, kernel_size_, kernel_d_, pad_,
		  pad_d_, stride_, stride_d_, bottom_diff + bottom[0]->offset(offset_indices));
	}
...
#ifdef CPU_ONLY
STUB_GPU(TridimConvolutionLayer);
#endif

INSTANTIATE_CLASS(TridimConvolutionLayer);
REGISTER_LAYER_CLASS(TridimConvolution);
