...
/**
 * @brief Fills a Blob with coefficients for trilinear interpolation.
 */
template <typename Dtype>
class TrilinearFiller : public Filler<Dtype> {
 public:
  explicit TrilinearFiller(const FillerParameter& param)
      : Filler<Dtype>(param) {}
  virtual void Fill(Blob<Dtype>* blob) {
    CHECK_EQ(blob->num_axes(), 4) << "Blob must be 4 dim.";
    CHECK_EQ(blob->width(), blob->height()) << "Filter must be square";
...
Filler<Dtype>* GetFiller(const FillerParameter& param) {
  ...
  if (type == "constant") {
    ...
    return new TrilinearFiller<Dtype>(param);
  } else {
