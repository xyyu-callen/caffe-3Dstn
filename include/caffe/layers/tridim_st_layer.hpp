#ifndef TRIDIM_ST_LAYER_HPP_
#define TRIDIM_ST_LAYER_HPP_
...
class TridimSpatialTransformerLayer : public Layer<Dtype> {

public:
	explicit TridimSpatialTransformerLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {
	...
	virtual inline const char* type() const { return "TridimSpatialTransformer"; }
...
private:
	...
	Dtype transform_forward_cpu(const Dtype* pic, Dtype pz, Dtype px, Dtype py);
	void transform_backward_cpu(Dtype dV, const Dtype* U, const Dtype pz, const Dtype px,
	...
	Dtype pre_defined_theta[12];
	bool is_pre_defined_theta[12];
